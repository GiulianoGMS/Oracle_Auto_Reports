CREATE OR REPLACE PROCEDURE NAGP_WTS_V2_RETURN_CM (psID NUMBER)
 ...
 AS
    ...
    FOR msg IN (
      ...
      SELECT A.ID, A.FONE, C.APIKEY, B.COMMAND, B.DATE_LOG, SUBSTR(B.ST, 0,300) ST
        FROM NAGT_ANSWERS_WTS A INNER JOIN NAGT_ANSWERS_WTS_LOG B ON A.ID = B.ID
                                INNER JOIN NAGT_API_CALL_NUMBERS C ON C.NROTELEFONE = A.FONE
                                WHERE a.ID = psID
                                  AND INDRETURN = 'N'
    )
    LOOP
      ...
      VTEXT := '%E2%9C%94%EF%B8%8F%20*Comando%20Executado:*%0A%0A' ||
        ...
        -- Construir a URL
        vUrl := 'http://api.textmebot.com/send.php?recipient=+'||msg.FONE||'&text=' || REPLACE(vText, ' ','%20') || '&apikey='||msg.APIKEY; -- Whatsapp 
